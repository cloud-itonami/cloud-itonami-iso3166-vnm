(ns marketentry.marketentryllm
  ...
  the VNM public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-vn-entity?=" (:has-vn-entity? e)
                        " mst-verified?=" (:mst-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-vn-entity? e))
                              (:has-vn-entity? e))
                          (or (not (:requires-mst? e))
                              (:mst-verified? e)))
                   0.9 0.3)}))
