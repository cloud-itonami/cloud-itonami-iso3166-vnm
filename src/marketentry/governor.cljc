(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Vietnam-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Vietnam-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-vn-entity?
                                       true` (near-universal for Vietnam
                                       ...
                                       Grounded in Vietnamese legal entity
                                       (MST) typically required for e-GP
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(MST/統一資格/muasamcong登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- vn-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-vn-entity? true`, INDEPENDENTLY verify
  `:has-vn-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-vn-entity?` ground truth (most Vietnam public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-vn-entity? e))
                 (not (true? (:has-vn-entity? e))))
        [{:rule :vn-entity-missing
          :detail (str subject " はベトナム居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- mst-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-mst? true`, INDEPENDENTLY check
  `:mst-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-mst? e))
                 (not (true? (:mst-verified? e))))
        [{:rule :mst-unverified
          :detail (str subject " はMST確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (vn-entity-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (mst-unverified-violations request st)
