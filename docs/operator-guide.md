...
2. Register the client's intake: business type, target public function,
   prior filing history in Vietnam if any.
3. Run the advisor in read-only mode against the Vietnam National E-Procurement System (VNEPS, muasamcong.mpi.gov.vn), overseen by the Ministry of Planning and Investment (MPI).
...
- named referral relationship with Vietnamese-licensed counsel or a
...
a working referral relationship with Vietnamese-licensed counsel or a
registered agent for whatever licensed representation the law of
Vietnam requires for actual public-procurement filings.
